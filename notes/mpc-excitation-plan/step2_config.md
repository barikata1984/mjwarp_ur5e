# Step 2: MPC Config データ構造

## 作成ファイル

- `src/mjwarp_ur5e/identification/mpc/__init__.py`
- `src/mjwarp_ur5e/identification/mpc/config.py`
...
class JointLimits:
    q_min: np.ndarray   # default: np.full(6, -2π)
    q_max: np.ndarray   # default: np.full(6, 2π)
    dq_max: np.ndarray  # default: [π,π,π,2π,2π,2π]
    ddq_max: np.ndarray # default: np.full(6, 8.0)
```

## 出力インタフェース

```python
# identification/mpc/config.py
...
```python
# identification/mpc/__init__.py (初期版、Step 6 で更新)

from .config import HorizonConfig, MPCConfig, PlannerConfig

__all__ = ["HorizonConfig", "MPCConfig", "PlannerConfig"]
```

## 設計意図

- `OptimizerConfig` (optimizer.py:31-58) のパターンに倣う
- `HorizonConfig` と `PlannerConfig` を分離し, `MPCConfig` がネストで保持
- `JointLimits` は既存のものをそのまま使う
- `q0` のデフォルトは UR5e ホームポジション (フランジ下向き)

## 検証

```python
# インポートテスト
from mjwarp_ur5e.identification.mpc import MPCConfig, HorizonConfig, PlannerConfig

# デフォルト生成
config = MPCConfig()
assert config.horizon.duration == 3.0
assert config.replan_period == 1.5
assert config.q0.shape == (6,)
assert config.num_joints == 6

# バリデーション
import pytest
with pytest.raises(ValueError):
    MPCConfig(replan_period=5.0, horizon=HorizonConfig(duration=3.0))
```
