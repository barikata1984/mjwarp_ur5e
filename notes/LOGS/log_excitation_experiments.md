...
## ペイロード初期姿勢・作業領域の再設定 (2026-06-02)

実機セットアップに合わせ home keyframe と作業領域を再定義. すべて base link 系 (UR5e base は world 原点・
identity なので world 座標 = base link 座標).

- **赤ブロックの向き**: 当初 body オフセット `quat` で 300 mm 辺を回していたが, オフセットを廃止し q5
  (wrist_3, 0-indexed) で向きを制御する方式に統一. q5=-14.1° で 300 辺が base x 軸沿い.
- **手先位置の IK 調整**: shoulder_pan で wrist_3 原点を base y 軸上 (x=0) に, さらに shoulder_lift/elbow/
  wrist_1 を数値 IK で解いて赤ブロック中心を作業領域中心 `[0, 0.55, 0.275]` に一致させた (ツール真下向き維持).
  最終 home (deg): `[75.90, -84.39, 85.99, -91.60, -90.00, -14.10]`.
- **作業領域** (`workspace_region_geom`, 最適化制約は実行時にこの geom から動的抽出):
  当初 x:[-0.3,0.3] y:[0.4,0.8] z:[0.05,0.5] → x/y 拡大して x:[-0.3,0.3] y:[0.2,0.9] z:[0.05,0.5] →
  最終 x 半幅を 0.25 に絞り **x:[-0.25,0.25] y:[0.2,0.9] z:[0.05,0.5]** (0.5 × 0.7 × 0.45 m).

## 速度・加速度制約スイープと評価 (2026-06-02)

m/b 分離 (`--with-ft-offset`, 16 列 regressor, 列スケーリング) を有効化し, 制約条件を振って最適励起軌道を
比較. 全条件 duration=5s, harmonics=5, n_monte_carlo=8, max_iter=150, objective=condition_number.
姿勢変化量は FT センサ座標系での重力方向の最大角差 (m·g 項と定数バイアス b を分離する条件, 目安 ≥45°).

**(A) 速度制限の比較** (x 半幅 0.3, ddq 制限なし):

| 条件 | cond | dq_max | ddq_max | 姿勢変化 | 安全性 |
|---|---|---|---|---|---|
| dq≤1.5 (EE なし) | 2.04 | 1.50 | 9.2 | 158° | 安全 |
| EE≤0.25 (dq なし) | 1.55 | **9.43** | 39.2 | 180° | **危険 (限界 3.14 の 3 倍)** |
| EE≤0.25 + dq≤3.0 | 1.63 | 3.00 | 14.3 | 167° | 安全 |
| EE≤0.25 + dq≤π/2 | 1.84 | 1.59 | 10.3 | 152° | 安全 (最も低速) |

- **EE 速度制約だけでは関節速度を縛れない**. EE 0.25 m/s でも特異点近傍で wrist が 7.65/9.43 rad/s に
  暴走し実機投入不可. dq 制限の併用が必須.

**(B) ddq スイープ** (x 半幅 0.3, EE≤0.25, dq≤π/2):

| ddq 上限 | cond | ddq_max 実測 | feasibility |
|---|---|---|---|
| 0.5π (1.57) | 1.95 | **5.96** | 破綻 (大違反) |
| 1π (3.14) | 1.91 | **8.67** | 破綻 (大違反) |
| **2π (6.28)** | **1.83** | 6.28 | ほぼ可 (境界のみ) |
| 3π (9.42) | 1.89 | 8.78 | ほぼ可 (境界のみ) |

- ddq を 2π 未満に絞ると, EE 0.25 + dq π/2 の下で"滑らかさ"と"励起の十分さ"が両立せず, optimizer が
  実行可能解を見つけられず加速度・速度を大きく違反した best-effort 解を返す. **ddq は 2π 以上が必須**.
  締めても condition number は改善せず (1.83→1.95 と悪化) 逆効果.

**(C) 作業領域 x 縮小** (EE≤0.25, dq≤π/2, ddq≤2π): x 半幅 0.3→0.25 で cond 1.831→1.865 (+1.9%, ほぼ不変).

**(D) EE 速度スイープ** (x 半幅 0.25, dq≤π/2, ddq≤2π):

| EE 上限 | cond | 姿勢変化 | dq_per_joint_max |
|---|---|---|---|
| 0.25 | **1.865** | 151° | ≤1.583 |
| 0.20 | 1.987 | 147° | ≤1.576 |
| 0.15 | 2.175 | 132° | ≤1.571 |
| 0.10 | 2.287 | 142° | ≤1.567 |

- **EE 速度を下げるほど condition number は単調に悪化** (0.25→0.10 で 1.87→2.29). 手先を遅くすると励起の
  質が落ちる. 0.25 m/s がこのグループの最良.

**総括**:
- m/b 分離の前提 (姿勢変化 ≫ 45°) は全条件で満たす (132–180°). cond は 1.55–2.29 の狭い範囲.
- 実機投入候補のベストは **EE≤0.25 + dq≤π/2 + ddq≤2π** (cond 1.83 @ x 半幅 0.3 / 1.865 @ x 半幅 0.25,
  関節速度 ≤1.58 rad/s, 加速度 ≤6.3 rad/s²). 全関節がハード限界 3.14 rad/s 内.
- 全条件で feasible=False だが違反は境界レベル (sub-mm / 0.01 オーダー). 厳密に feasible 化するなら
  safety_margin を緩めるか dq-max を 1.55 等にマージン取りする.
- 成果物: `results/ddq_2pi/`, `results/ddq_2pi_x025/`, `results/best_ddq2pi/`, `results/ee0{10,15,20}/`,
  比較動画・プロット `results/comparison/`. 旧散在 51 ファイルは削除済み.

## 作業領域の縮小が feasibility と同定品質に与える影響 (2026-06-03)

EE 速度スイープ (0.25/0.20/0.15/0.10, dq≤π/2, ddq≤2π) を作業領域サイズを変えて 3 セット実施し,
有効解 (境界違反のみ) と破綻解 (実行可能解が見つからず制約を大きく違反した best-effort 解) を区別.

| 作業領域 (x半幅, y半幅) | EE0.25 | EE0.20 | EE0.15 | EE0.10 | 有効/4 |
|---|---|---|---|---|---|
| x0.25, y0.35 | 1.87 | 1.99 | 2.17 | 2.29 | **4/4** |
| x0.25, y0.30 | 1.89 | 破綻 | 2.25 | 破綻 | 2/4 |
| x0.20, y0.20 | 2.12 | 破綻 | 2.63 | 破綻 | 2/4 |

- **作業領域を狭めるほど condition number は単調に悪化**し (有効解の EE0.25 で 1.87→1.89→2.12),
  かつ **feasibility が崩れやすくなる** (y0.35 では 4/4 収束, y0.30・xy0.20 では 2/4 が破綻).
- 破綻解は cond が低く見えても (例 y0.30/EE0.10 で cond 1.62) 中身は関節速度 5.73 rad/s・加速度 32 rad/s²・
  EE 速度 3.14 m/s と全制約を激しく違反した暴走軌道で, condition number の値に意味はない. 低 cond の
  非実行可能解を"良い結果"と誤認しないこと.
- **教訓**: 励起には十分な作業空間が必要. 作業域を絞るのは同定品質・実行可能性の両面で不利.
  破綻が増えた場合は n_monte_carlo を増やすか, 作業領域を広げる.

### results 整理方針

実機投入可能性で選別. **破綻解** (制約を境界 ≲0.05 を超えて大違反) と **実機投入不可解** (関節速度が
